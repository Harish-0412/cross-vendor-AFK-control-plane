...
The Odysseus project has multiple components with varying release cycles but tight-coupled dependencies:
...
All published packages use `@odysseus/*` npm scope:
- `@odysseus/protocol`
- `@odysseus/schemas`
- `@odysseus/config`
...
```
@odysseus/config → @odysseus/schemas → @odysseus/protocol
                        ↗
gateway/core → gateway/adapters/* → @odysseus/schemas
gateway/sandbox → @odysseus/protocol
...
- Path aliases `@odysseus/*` for IDE resolution
...
- Predictable import paths `@odysseus/protocol`
