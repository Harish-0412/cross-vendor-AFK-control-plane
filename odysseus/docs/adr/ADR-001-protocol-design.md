...
The Odysseus AFK Control Plane requires a normalized event and command protocol to enable consistent, type-safe communication between:
...
All protocol types have corresponding Zod schemas in `@odysseus/schemas`. Every boundary must validate:
