...
`;
    const normalText =
      'This is normal text without any secrets. It contains words like password, secret, token, key but they are not actual credentials. The system should not redact these false positives.';

    return (
      baseText.repeat(Math.ceil((sizeKB * 1024) / baseText.length)) +
      normalText.repeat(Math.ceil((sizeKB * 1024) / normalText.length))
    );
  };

  it('should redact 1KB in under 10ms at steady state', () => {
    const text = generateText(1);
    // The suite runs packages concurrently. A single first-call sample
    // measures scheduler/JIT noise as often as redaction performance.
  ...
  it('should handle streaming redaction', async () => {
    ...
    const chunks = Array.from(
      { length: 100 },
      (_, i) => `Line ${i}: apiKey=sk_live_abcdefghijklmnopqrstuvwxyz123456 password=secret123\n`,
    );
