import { type CustomPattern, type SecretType } from './types';
...
export const BUILTIN_PATTERNS: CustomPattern[] = [
  ...
  },
  {
    ...
    pattern:
      /(?:aws[_-]?secret[_-]?access[_-]?key|aws_secret_key)\s*[:=]\s*["']?([A-Za-z0-9/+=]{40})["']?/gi,
    type: 'aws_secret_key',
  ...
  // GCP
  {
    ...
    pattern:
      /"type":\s*"service_account"[\s\S]{100,?"private_key":\s*"-----BEGIN PRIVATE KEY-----/gi,
    type: 'private_key',
  ...
  // Passwords
  {
    ...
    pattern:
      /(?:password|passwd|pwd|pass|secret|api[_-]?secret)\s*[:=]\s*["']?([^"'\s]{8,})["']?/gi,
    type: 'password',
