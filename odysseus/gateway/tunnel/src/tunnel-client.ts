...
import { DEFAULT_TUNNEL_CONFIG, classifyAuthFailure } from './types';
...
export class TunnelClient {
  ...
  private healthyConnectionTimer?: NodeJS.Timeout | undefined;
  private lastFailure: TunnelFailure | null = null;
  ...
  private heartbeatContributor: (() => Partial<HeartbeatPayload>) | null = null;
  ...
  private async openWebSocket(): Promise<void> {
    ...
    if (!WS) {
      // This used to fake a connection and report `connected`. A transport
      // that silently pretends to work is the worst possible failure mode:
      // the gateway believed it was online while nothing crossed the wire.
      // Tests inject a mock explicitly via setWebSocketImplementation().
      throw new Error(
        'No WebSocket implementation configured. Call setWebSocketImplementation() before connect().',
      );
    }

    const url = this.config.controlPlaneUrl;
    const protocols = 'odysseus-tunnel.v1';
  ...
  private sendAuthMessage(): void {
    if (!this.authProvider) {
      // Previously this declared itself connected without sending anything —
      // another false green: against a real Control Plane the socket is closed
      // with 4001 on the first non-auth frame. The server requires deviceId and
      // gatewayId; signing is an additional proof, not a precondition. So send
      // a real, unsigned auth message and let the server decide.
      this.sendRaw('auth', {
  ...
  /**
   * Reset the reconnect budget only once the connection has stayed up long
   * enough to count as healthy. Until then a flapping connection keeps its
   * accumulated backoff, so repeated instant failures genuinely back off
   * instead of hammering.
   */
  private startHealthyConnectionTimer(): void {
    this.stopHealthyConnectionTimer();
    const threshold = this.config.healthyConnectionMs;
    if (threshold <= 0) {
    ...
    this.healthyConnectionTimer = setTimeout(() => {
      if (this._state === 'connected') {
        ...
        this.emitEvent({
          ...
          timestamp: new Date(),
          message: `Connection healthy for ${threshold}ms; backoff reset`,
        });
      }
    }, threshold);
    if (typeof this.healthyConnectionTimer.unref === 'function') {
      this.healthyConnectionTimer.unref();
    }
  }

  private stopHealthyConnectionTimer(): void {
  ...
  private handleAuthSuccess(payload: AuthSuccessPayload): void {
    ...
    // The backoff is NOT reset here. A connection that authenticates and then
    // dies half a second later would otherwise reset the counter every time,
    // producing a tight reconnect loop that still reports as "connected".
    this.startHealthyConnectionTimer();
    this.emitEvent({
      type: 'auth_success',
      timestamp: new Date(),
      payload: {
  ...
  /**
   * Apply the failure taxonomy to an auth rejection.
   *
   * The classification decides everything: a revoked device must stop dead
   * (its credentials will never start working, and retrying forever hides the
   * revocation from the operator), while a device that is merely awaiting
   * approval in the UI must retry slowly rather than spin.
   */
  private handleAuthFailure(
    ...
    this.emitEvent({
      type: failure.class === 'auth_fatal' ? 'certificate_warning' : 'auth_failure',
      timestamp: new Date(),
      message: `${failure.code}: ${failure.reason} (class=${failure.class})`,
      payload: failure,
    });
    ...
    if (!failure.retryable) {
      ...
      this.emitEvent({
        ...
        timestamp: new Date(),
        message:
          failure.class === 'auth_fatal'
            ? `Device credentials rejected permanently (${failure.code}); not reconnecting`
            : `Authentication failed (non-retryable): ${failure.code} - ${failure.reason}`,
        payload: failure,
      });
      return;
    }

    this.handleConnectionFailure(failure.retryAfterMs);
  }

  /** The most recent classified failure, for supervisors deciding how to react. */
  getLastFailure(): TunnelFailure | null {
    return this.lastFailure;
  }

  /** The resolved configuration, including defaults. Read-only. */
  ...
  private async handleIncomingCommand(msg: TunnelMessage): Promise<void> {
    ...
    // The Control Plane correlates acks by the TOP-LEVEL `correlationId` field
    // (see TunnelServer.handleSocketMessage), not by anything inside the
    // payload. `send()` places it there; `sendRaw()` does not — using sendRaw
    // here leaves every command unacked until the server's 10s timeout fires.
    ...
    try {
      ...
      if (correlation) {
        this.send(
          'ack',
          { correlationId: correlation, success: false, error: message },
          correlation,
        );
      }
    }
   ...
   *
   * Exponential growth to a cap, then randomised between 0.5x and 1.5x of that
   * value — the policy the `nats` CLI uses. The randomisation is the important
   * part: without it, a Control Plane restart makes every gateway in the fleet
   * reconnect in lockstep and the resulting thundering herd can stop the
   * Control Plane from ever becoming healthy again.
   *
   * TLS links get wider spread because a TLS handshake is far more expensive
   * for the server than a plaintext one, so simultaneous retries hurt more.
   */
  private computeReconnectDelay(attempt: number, explicitDelayMs?: number): number {
    if (explicitDelayMs !== undefined) {
      // A server-directed delay is still jittered, or every client told
      // "come back in 5s" returns at precisely the same instant.
      return this.applyJitter(explicitDelayMs);
    }
    const exponential = this.config.reconnectBaseMs * Math.pow(2, Math.max(0, attempt - 1));
    return this.applyJitter(Math.min(exponential, this.config.reconnectMaxMs));
  }

  private applyJitter(delayMs: number): number {
    const isTls = this.config.controlPlaneUrl.startsWith('wss:');
    // Randomise within ±50% for TLS, ±25% otherwise.
    const spread = isTls ? 0.5 : 0.25;
  ...
  private async reconnect(explicitDelayMs?: number): Promise<void> {
    ...
    // A negative budget means unlimited — the default for a long-lived daemon.
    const budget = this.config.maxReconnectAttempts;
    if (budget >= 0 && this.reconnectAttempts >= budget) {
      this.emitEvent({
        type: 'reconnect_failed',
        timestamp: new Date(),
        message: `Max reconnect attempts (${budget}) reached`,
      });
      this.setState('error');
      return;
    }
    ...
    const delay = this.computeReconnectDelay(this.reconnectAttempts, explicitDelayMs);

    this.emitEvent({
      type: 'reconnect_attempt',
      timestamp: new Date(),
      message: `Reconnect attempt ${this.reconnectAttempts}${
        budget >= 0 ? `/${budget}` : ''
      } in ${delay}ms`,
      payload: { attempt: this.reconnectAttempts, delayMs: delay },
    });
  ...
  /**
   * Register a source of live telemetry for heartbeats (admission phase, load).
   * The tunnel has no view of gateway internals, so the gateway pushes this in.
   */
  setHeartbeatContributor(contributor: (() => Partial<HeartbeatPayload>) | null): void {
    this.heartbeatContributor = contributor;
  }

  /**
   * Send a heartbeat immediately, outside the regular interval.
   *
   * Used when something the Control Plane routes on has changed — chiefly the
   * admission phase. Waiting up to a full interval to announce "I am draining"
   * means the router keeps sending work to a gateway that will refuse it.
   */
  sendImmediateHeartbeat(): void {
    if (this._state !== 'connected') return;
    this.sendHeartbeat();
  }

  private sendHeartbeat(): void {
    const contributed = this.heartbeatContributor?.() ?? {};
