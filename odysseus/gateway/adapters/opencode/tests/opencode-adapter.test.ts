...
import type { Sandbox, SessionConfig } from '@odysseus/protocol';
...
describe('OpenCodeAdapter', () => {
  ...
  it('collects the real unified diff in the session project and is honest about unsupported approvals', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'odysseus-opencode-'));
    try {
      await execFileAsync('git', ['init'], { cwd: projectRoot });
      await execFileAsync('git', ['config', 'user.email', 'test@odysseus.dev'], { cwd: projectRoot });
      await execFileAsync('git', ['config', 'user.name', 'Odysseus Test'], { cwd: projectRoot });
