...
describe('history sync', () => {
  ...
  beforeEach(() => {
    home = createHome();
    writeFile(
      join(home.home, '.codex', 'sessions', '2026', '09', '10', `rollout-2026-09-10T10-00-00-${SESSION_ID}.jsonl`),
      [
        j({ timestamp: '2026-09-10T10:00:00Z', type: 'session_meta', payload: { id: SESSION_ID, timestamp: '2026-09-10T10:00:00Z', cwd: join(home.home, 'project') } }),
        j({ timestamp: '2026-09-10T10:00:03Z', type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: 'Hello Codex' }] } }),
        j({ timestamp: '2026-09-10T10:00:08Z', type: 'event_msg', payload: { type: 'token_count', info: { total_token_usage: { total_tokens: 50 } }, rate_limits: { primary: { used_percent: 5, window_minutes: 300, resets_at: 1789990846 } } } }),
        '',
      ].join('\n'),
    );
  ...
  it('lists conversations with ~ workspace paths and no usage when only history is granted', async () => {
    ...
    expect(summary).toMatchObject({ externalId: SESSION_ID, title: 'Hello Codex', workspace: '~/project' });
  ...
  it('adds token totals and plan limits when usage is granted too', async () => {
    ...
    expect(usage && usage.kind === 'usage_snapshot' && usage.snapshot.windows?.[0]?.usedPercent).toBe(5);
  });
  ...
  it('cannot be steered to another file by a crafted conversation id', async () => {
    await grant(['history.read']);
    for (const id of ['../../auth.json', '..\\..\\auth.json', join(home.home, '.codex', 'auth.json'), 'auth']) {
