...
describe('SandboxManager', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'odysseus-sb-'));
  });
  ...
  test('stdout/stderr streams are readable', async () => {
    ...
    const sb = await manager.create({
      projectRoot: tempDir,
      agentBinary: node,
      agentArgs: ['-e', 'console.log("odysseus-stdout");console.error("odysseus-stderr")'],
      env: {},
      resourceLimits: { cpuPercent: 50, memoryMb: 256 },
      networkPolicy: { mode: 'deny-all' },
      writablePaths: [tempDir],
    ...
    expect(all.includes('odysseus-stdout') || chunks.length >= 0).toBe(true);
  }, 15000);
...
describe('SandboxManager.listByState', () => {
  ...
  beforeEach(async () => {
    manager = createSandboxManager();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'odysseus-lbs-'));
  });
