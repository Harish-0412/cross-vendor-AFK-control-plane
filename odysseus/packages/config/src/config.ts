import type { GatewayOptions, ResourceLimits } from '@odysseus/protocol';
import { validateGatewayOptions } from '@odysseus/schemas';
...
export function mergeGatewayOptions(
  ...
  for (const opts of options) {
    ...
    // Control Plane config is absent by default (local-only gateway), so it is
    // merged field-by-field onto whatever a previous layer supplied.
    if (opts.controlPlane) {
      ...
      if (c.heartbeatIntervalMs !== undefined) dst.heartbeatIntervalMs = c.heartbeatIntervalMs;
      if (c.reconnectBaseMs !== undefined) dst.reconnectBaseMs = c.reconnectBaseMs;
      if (c.reconnectMaxMs !== undefined) dst.reconnectMaxMs = c.reconnectMaxMs;
      if (c.maxReconnectAttempts !== undefined) dst.maxReconnectAttempts = c.maxReconnectAttempts;
      merged.controlPlane = dst;
    }
  }
