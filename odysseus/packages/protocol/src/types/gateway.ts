...
export interface GatewayOptions {
  ...
  /**
   * Control Plane tunnel settings. When `url` is set the gateway dials the
   * Control Plane over WSS and forwards its event stream there; when it is
   * omitted the gateway runs purely local (no tunnel, no remote control).
   */
  controlPlane?: {
    url: string;
    autoConnect?: boolean;
