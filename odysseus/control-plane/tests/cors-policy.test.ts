...
describe('CORS policy', () => {
  ...
  describe('with localhost allowlist (default frontend dev)', () => {
    beforeEach(async () => {
      cp = new ControlPlane({ port: 0, corsOrigins: ['http://localhost:3000'] });
      const { url } = await cp.start();
      baseUrl = url;
    });
    ...
    it('handles preflight OPTIONS request with credentials and custom headers', async () => {
      ...
      expect(res.headers.get('access-control-allow-credentials')).toBe('true');
      expect(res.headers.get('access-control-allow-headers')).toBe('Content-Type, X-Client-Type');
    });

    it('allows 127.0.0.1 variant for local development', async () => {
