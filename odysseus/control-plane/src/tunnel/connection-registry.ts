...
export class ConnectionRegistry {
  ...
  /** Record the phase a gateway reported on its latest heartbeat. */
  setAdmissionPhase(deviceId: string, phase: GatewayAdmissionPhase): void {
    const conn = this.gatewayConnections.get(deviceId);
    if (conn) conn.admissionPhase = phase;
  }

  getAdmissionPhase(deviceId: string): GatewayAdmissionPhase {
    return this.gatewayConnections.get(deviceId)?.admissionPhase ?? 'running';
  }
