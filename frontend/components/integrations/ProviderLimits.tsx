...
import { DollarSign, Gauge, Info, Loader2 } from "lucide-react";
import {
  Card,
  CardContent,
  CardDescription,
  ...
  return (
    <div className={compact ? "space-y-3" : "grid gap-4 md:grid-cols-2"}>
      {entries.map((entry) => {
        ...
        const allReset =
          windows.length > 0 &&
          windows.every((window) => window.resetSinceReading);
        const organization = entry.snapshot.organization;
        return (
          <Card key={`${entry.deviceId}:${entry.integration}`}>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                {organization ? "Reported by OpenAI" : "As recorded by Codex"}{" "}
                {formatRelative(entry.snapshot.observedAt, now)} (
            ...
            <CardContent className="space-y-4">
                    ...
                    <div className="rounded-lg border p-3">
                      ...
                      <p className="mt-1 text-xl font-semibold tabular-nums">
                        {new Intl.NumberFormat(undefined, {
                          style: "currency",
                          currency: organization.currency.toUpperCase(),
                        }).format(organization.totalCost)}
                      </p>
                    </div>
                    <div className="rounded-lg border p-3">
                      <p className="text-xs text-muted-foreground">
                      ...
                      <p className="mt-1 text-xl font-semibold tabular-nums">
                        {organization.requests.toLocaleString()}
                      </p>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                      ...
                      <p className="font-medium tabular-nums">
                        {organization.inputTokens.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-medium tabular-nums">
                        {organization.cachedInputTokens.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Output</span>
                      <p className="font-medium tabular-nums">
                        {organization.outputTokens.toLocaleString()}
                      </p>
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Provider-reported Costs and Usage API totals for{" "}
                    {new Date(organization.periodStart).toLocaleDateString()}–
                    {new Date(organization.periodEnd).toLocaleDateString()}. No
                    price estimates are added.
                  </p>
                </>
              )}
              {!organization && windows.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  Codex did not record any usage windows.
                </p>
              )}
              {windows.map((window) => (
                  ...
                  <div className="flex items-baseline justify-between text-sm">
                    ...
                    {window.resetSinceReading ? (
                      ...
                      <span className="tabular-nums">
                        ...
                        {Math.max(0, 100 - window.usedPercent)}% left
                      </span>
                    )}
                  </div>
                  {window.resetSinceReading ? (
              ...
              {entry.snapshot.credits && !entry.snapshot.credits.unlimited && (
                <div className="flex items-center justify-between border-t pt-3 text-sm">
                  ...
                  <span className="tabular-nums">
                    {entry.snapshot.credits.balance ?? "—"}
                  </span>
                </div>
              )}
              ...
              {allReset && (
                <p className="flex items-start gap-2 rounded-md bg-muted/50 p-2.5 text-xs text-muted-foreground">
                  ...
                  Every window has reset since Codex last ran, so the figures
                  above are not shown as current. Use Codex once and they will
