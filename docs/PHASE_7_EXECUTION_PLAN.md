...
| §11.5 Kill switch | ✅ **Implemented (Subphase 7.6).** `POST /api/v1/devices/:deviceId/kill-switch` (fan-out cancel of every active session) and `POST /api/v1/devices/:deviceId/lock` (observation-only via `trustProfile: 'locked'`) are implemented, ownership-checked, audited, and covered by `tests/phase7-kill-switch.test.ts` (5 tests). Frontend: global STOP ALL / LOCK ALL in the app header (persistent chrome) + per-device controls on the devices page. |
