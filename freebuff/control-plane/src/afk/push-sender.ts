...
export class PushSender {
  ...
  constructor(private readonly db: IDatabase, options: PushSenderOptions = {}) {
    ...
    const subject = options.vapidSubject ?? process.env.VAPID_SUBJECT ?? 'mailto:admin@freebuff.dev';
    ...
    this.webPushConfigured = Boolean(
      options.webPushTransport || (publicKey && privateKey),
    );
  ...
  async sendToUser(userId: string, notification: PushNotification): Promise<PushDeliveryResult[]> {
    ...
    return Promise.all(subscriptions.map(async (subscription): Promise<PushDeliveryResult> => {
      try {
        ...
        return { subscriptionId: subscription.id, channel: subscription.channel, delivered: true };
      } catch (error) {
        return {
