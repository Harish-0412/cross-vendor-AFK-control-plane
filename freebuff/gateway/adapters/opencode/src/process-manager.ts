...
export class OpenCodeProcessManager implements OpenCodeProcessController {
  ...
  constructor(
    private readonly sandboxManager: ISandboxManager = new SandboxManager(),
  ...
  async validate(): Promise<{ valid: boolean; version?: string; errors: string[] }> {
    ...
    if (!detected)
      return { valid: false, errors: [`OpenCode binary not found: ${this.binaryPath}`] };
    if (compareVersions(detected.version, this.minimumVersion) < 0) {
      return {
        valid: false,
        version: detected.version,
        errors: [`OpenCode ${this.minimumVersion}+ is required; found ${detected.version}`],
      };
    }
  ...
  watchStdout(sandbox: Sandbox, onLine: (line: string) => void, onClose: () => void): void {
    ...
    void once(sandbox.stdout, 'end').then(() => {
      lines.close();
      onClose();
    });
  }
